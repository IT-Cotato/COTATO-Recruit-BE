...
class SaveAnswersControllerTest {
	...
	@Test
	@DisplayName("지원서 질문에 대한 응답을 임시저장한다")
	@WithMockCustomUser
	void saveAnswers() throws Exception {
		// given
		AnswerRequest answerRequest = new AnswerRequest(1L, "백엔드 개발자가 되고 싶습니다.");
		PartAnswerRequest request =
				new PartAnswerRequest(List.of(answerRequest), "http://pdf.url", "pdf-key");

		willDoNothing()
				.given(applicationAnswerService)
				.saveAnswers(anyLong(), anyLong(), anyList(), anyString(), anyString());

		// when & then
		mockMvc.perform(
						post("/api/applications/{applicationId}/answers", 1L)
